#![crate_name = "payjoin_ffi_test_utils"]
...
uniffi::setup_scaffolding!("payjoin_test_utils");
...
#[derive(uniffi::Object)]
pub struct TestServices(Mutex<payjoin_test_utils::TestServices>);
...
impl TestServices {
    ...
    pub fn fetch_ohttp_keys(&self) -> Result<payjoin_ffi::OhttpKeys, BoxSendSyncError> {
        let runtime = RUNTIME.lock().expect("Lock should not be poisoned");
        runtime.block_on(async {
            self.0
                .lock()
